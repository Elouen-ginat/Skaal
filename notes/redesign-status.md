...
- [x] 3.x `Store[T, B]` / `Relational[B]` / `BlobStore[B]` / `Channel[T, B]` backend generic populating `ResourceOverrides.backend` *(`Store`/`BlobStore`/`Channel` landed in Phase 4 §4.4; `Relational[B]` followed on this branch as a single-generic shape — the class body is the row schema, no companion DTO — sitting on `SQLModel` + `Generic[B]` whose `__class_getitem__` stashes `__skaal_backend_pin__` onto the parametrised class. `SQLModelMetaclass` overwrites `__orig_bases__` on subclasses, so the decorator's `_extract_backend_pin` now reads `__skaal_backend_pin__` (MRO-inherited) ahead of the `__orig_bases__` walk used by the other primitives. The ADR 028 §6.5 two-generic example is stale and needs a follow-up edit.)*
