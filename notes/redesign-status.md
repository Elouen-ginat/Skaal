...
- [x] 3.x `Store[T, B]` / `Relational[T, B]` / `BlobStore[B]` / `Channel[T, B]` second generic populating `ResourceOverrides.backend` *(`Store`/`BlobStore`/`Channel` landed in Phase 4 §4.4; `Relational[T, B]` followed on this branch — a typed `Relational` base sitting on `SQLModel` + `Generic[T, B]` whose `__class_getitem__` stashes `__skaal_value_type__` / `__skaal_backend_pin__` onto the parametrised class. `SQLModelMetaclass` overwrites `__orig_bases__` on subclasses, so the decorator's `_extract_backend_pin` now reads `__skaal_backend_pin__` (MRO-inherited) ahead of the `__orig_bases__` walk used by the other primitives.)*
...
- [x] 4.4 Decorator rewire: `Store[T, B]` / `Relational[T, B]` / `BlobStore[B]` / `Channel[T, B]` second generic flows into `ResourceOverrides.backend`. `Relational[T, B]` landed after the other primitives — `SQLModelMetaclass` strips `__orig_bases__` from subclasses, so the `Relational` base captures both type args in `__class_getitem__` (`__skaal_value_type__` + `__skaal_backend_pin__` on the parametrised class, inherited via MRO) and `_extract_backend_pin` reads `__skaal_backend_pin__` ahead of the existing `__orig_bases__` walk.
...
- [x] `uv run ruff check && uv run mypy skaal && uv run pytest` green (332 tests; mypy clean on 139 source files; ruff clean)
