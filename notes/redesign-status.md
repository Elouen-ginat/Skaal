...
**Last updated:** 2026-05-15 — Phase 4 deploy-build foundation landed on `claude/continue-redesign-1u5Bv`: a new `skaal.deploy` package ships `build_artefacts(bound, env, app_spec)` (Jinja2 templating into `./.skaal/build/<env>/`), `tags_for(resource, env, fingerprint)` returning a typed `SkaalTags` pydantic model, and the AWS template tree (`Dockerfile.j2`, `handler.py.j2`, `bootstrap.py.j2`, `pyproject.toml.j2`). Every build-time data structure is typed: `BuildContext`, `BuildManifest`, `ManifestResourceEntry`, `BuildPyProject`, and `SkaalTags` are frozen pydantic models with `extra="forbid"`. The rendered Lambda image installs its dependencies via `uv pip install -r pyproject.toml` (no `requirements.txt`; every transitive dep flows through skaal's `[project.optional-dependencies]` table). `SourceLocation` grew `top_package` / `bare_name` properties so every consumer reads the structured fields rather than re-splitting `<module>:<qualname>`. `skaal build` is wired against the new pipeline (no longer a Phase 1 stub). The Pulumi-program callable (`pulumi_program_for`), per-backend AWS synth modules, and `skaal deploy` rewire remain pending and land on a follow-up branch. The previous milestone (2026-05-14) landed the Phase 4 dunder sweep, ASGI mount alias deletion, and `skaal plan` rewire on `claude/continue-redesign-SbweQ`.
...
- [ ] 4.2 `skaal/deploy/` rebuilt: `pulumi_program_for(bound, app, env)`, `build_artefacts(...)`, per-backend AWS synth modules, `tags_for()` helper *(partial — `build_artefacts(...)` and `tags_for(...)` landed; the Pulumi-program callable and per-backend AWS synth modules are still pending)*
...
- [x] 4.8 `skaal run`, `skaal plan <module:attr>`, and `skaal build` are wired against `infer → bind`; `skaal deploy` is still stubbed (the plan-as-diff form is Phase 6)
...
- [x] `uv run ruff check && uv run mypy skaal && uv run pytest` green (288 tests; mypy clean on 120 source files; ruff clean)
