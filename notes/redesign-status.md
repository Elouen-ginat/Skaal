...
**Last updated:** 2026-05-15 — Phase 4 deploy-build foundation landed on `claude/continue-redesign-1u5Bv`: a new `skaal.deploy` package ships `build_artefacts(bound, app, env)` (Jinja2 templating into `./.skaal/build/<env>/`) and `tags_for(resource, env, fingerprint)` (the canonical `skaal:*` tag set), plus the AWS template tree (`Dockerfile.j2`, `handler.py.j2`, `bootstrap.py.j2`, `requirements.txt.j2`). `skaal build` is wired against the new pipeline (no longer a Phase 1 stub) and writes a `manifest.json` recording the rendered resource set + plan fingerprints. The Pulumi-program callable (`pulumi_program_for`), per-backend AWS synth modules, and `skaal deploy` rewire remain pending and land on a follow-up branch. The previous milestone (2026-05-14) landed the Phase 4 dunder sweep, ASGI mount alias deletion, and `skaal plan` rewire on `claude/continue-redesign-SbweQ`.
...
- [x] 4.2 Jinja2 templates under `skaal/deploy/templates/aws/` (`Dockerfile.j2`, `handler.py.j2`, `bootstrap.py.j2`, `requirements.txt.j2`)
...
- [x] `uv run ruff check && uv run mypy skaal && uv run pytest` green (271 tests; mypy clean on 119 source files; ruff clean)
