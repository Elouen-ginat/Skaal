...
- The full typed primitive surface (`Store[T, B]`, `Relational[B]`, `BlobStore[B]`, `Channel[T, B]` with `B` defaulted to `Backend`). The backend generic parameter depends on the `Backend` token tree that Phase 3 ADR 031 builds; Phase 2 keeps the existing single-parameter generics (`Store[T]`, `Channel[T]`, `BlobStore`) and stubs in a non-functional `Backend` marker so Phase 3 can flesh it out without an `__init__.py` re-cut.
