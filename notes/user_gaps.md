...
ADR 014 removed public HTTP routing/streaming from the "what Skaal should build next" list, and ADRs 015, 016, 018, 021, 022, 023, 024, 025, and 026 landed the first coherent pass on storage surface, blob storage, agent persistence, solver diagnostics, catalog overlays, relational migrations, secret injection, per-row TTL, and background jobs. The next coherent pass is now the remaining examples/runtime ergonomics and backend-efficiency work.
...
1. **Examples ladder and testing story** — the examples still do not cover agents, schedules, patterns, or test fixtures, and there is still no first-party `skaal.testing` helper or pytest fixture surface. P1. ([§A.2](#a2-testing-story), [§A.8](#a8-examples-dont-progress-and-miss-common-patterns))
2. **Backend-native cursor/index optimization** — `Store[T]` now has the right API, but Firestore and Redis still materialize too much per page or index query, DynamoDB index lookup still drains bucket state before slicing, and SQL backends still do not provision native secondary indexes from declared metadata. P1 scalability gap. ([§B.2](#b2-kv-store-and-storage-tiers), [ADR 027](./design/027-backend-native-cursor-index-optimization-plan.md))
3. **Schedule coordination in multi-replica deploys** — `@app.schedule` still has no framework-level distributed lock or exposed overlap/misfire policy knobs, so correctness depends on the deploy target and process topology. P0 once users scale past one replica. ([§B.8](#b8-schedules-skaalschedulepy-moduleschedule))
4. **Vector retrieval beyond pure similarity search** — the vector tier still lacks a first-class hybrid search and re-embedding story, which blocks realistic search and RAG apps from staying inside Skaal. P1. ([§B.4](#b4-vector-tier-skaalvectorpy))
5. **Read-path operational polish for modules and tests** — module exports remain IDE-opaque `dict[str, Any]` namespaces, and users still have no documented test harness for in-memory runtime composition. P2 individually, but high cumulative friction. ([§A.2](#a2-testing-story), [§A.9](#a9-module-mounting-and-cross-module-references))
6. ~~**Blob / object storage tier**~~ — landed in [ADR 016](./design/016-blob-storage-tier-implementation-plan.md): Skaal now has `BlobStore`, `@app.storage(kind="blob")`, and bundled file/S3/GCS backends.
7. ~~**Agent persistent-state save/load**~~ — landed in [ADR 018](./design/018-agent-persistence-implementation-plan.md): persistent fields now load/save per invocation and same-identity calls serialize inside `LocalRuntime`.
8. ~~**Solver-failure error messages with closest-match suggestions**~~ — landed in [ADR 021](./design/021-solver-diagnostics-implementation-plan.md): UNSAT paths now render candidate diagnostics, closest matches, and relax suggestions.
9. ~~**Catalog overrides per environment**~~ — landed in [ADR 022](./design/022-catalog-overrides-implementation-plan.md): `[skaal] extends`, removal overlays, `skaal catalog validate`, and `skaal catalog sources` now cover the per-environment flow.
10. ~~**Relational migrations beyond `create_all`**~~ — landed in [ADR 023](./design/023-relational-migrations-implementation-plan.md): Alembic-driven `skaal migrate relational` with `autogenerate`, `upgrade`, `downgrade`, `current`, `history`, `check`, `stamp` and a typed Python API.
11. ~~**Secret injection at deploy / runtime**~~ — landed in [ADR 024](./design/024-secret-injection-implementation-plan.md): deploy builders and runtime resolvers now wire AWS Secrets Manager / GCP Secret Manager.
12. ~~**Per-row TTL / cache semantics**~~ — landed in [ADR 025](./design/025-per-row-ttl-implementation-plan.md): `@app.storage(retention=...)` now applies runtime expiry, `Store.set`/`add`/`update` accept `ttl=`, and the solver enforces backend TTL capability.
13. ~~**Long-running / delayed background jobs**~~ — landed in [ADR 026](./design/026-background-jobs-implementation-plan.md): `@app.job` plus `enqueue(...)` now cover immediate and delayed background work.
...
| Want                                       | Today                                                                                                       | Sev    |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | ------ |
| Pagination / cursor on `Store[T]`          | `Store.list_page(...)` and `scan_page(...)` now exist, and SQLite/Postgres/DynamoDB already push the primary traversal into backend-native resume tokens or ordered queries. The remaining gap is that Firestore still streams then slices results in-process, and Redis still walks sorted-set state plus payload fetches rather than using a tighter native cursor path. See [ADR 027](./design/027-backend-native-cursor-index-optimization-plan.md). | P1 |
| Secondary indexes on `Store[T]`            | `SecondaryIndex(...)` and `Store.query_index(...)` now exist, but SQL backends still rely on JSON-expression scans without provisioning native secondary indexes, Redis still drains whole index buckets before slicing, and DynamoDB / Firestore still use bucket-materialization rather than native GSI / field-query paths. See [ADR 027](./design/027-backend-native-cursor-index-optimization-plan.md). | P1 |
