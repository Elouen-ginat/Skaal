...
def generate_artifacts(
    ...
    readme_path.write_text(textwrap.dedent(f"""\
        ...
        - Pulumi CLI installed (`pip install pulumi`)
        ...
        # 2. Deploy infrastructure (no Python SDK needed — uses Pulumi YAML runtime)
        ...
        ```bash
        pulumi config set lambdaMemoryMb 512   # default from catalog
        pulumi config set lambdaTimeout 60
        pulumi config set lambdaRuntime python3.12
        ```
