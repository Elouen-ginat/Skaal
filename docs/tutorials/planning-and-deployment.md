...
- Read [Python API: CLI-Parity API](../reference/python-api-cli-parity.md) for the `skaal.api` functions that mirror the landed `init`, `run`, `plan`, `map`, `where`, `trace`, `build`, `deploy`, `stubs`, and `doctor` commands.
