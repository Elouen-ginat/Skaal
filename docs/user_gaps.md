...
9. ~~**Per-row TTL / cache semantics**~~ — landed in [ADR 025](./design/025-per-row-ttl-implementation-plan.md): `@app.storage(retention=...)` now applies runtime expiry, `Store.set`/`add`/`update` accept `ttl=`, and the solver enforces backend TTL capability.
