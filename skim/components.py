...
and wired in via ``app.attach(component)``. They appear in ``plan.skim.lock``
...
from skim.types import AccessPattern, Durability, Latency, RateLimitPolicy, Throughput
...
class ExternalComponent(ComponentBase):
    ...
    1. Writes their spec into ``plan.skim.lock`` as a reference.
